...
<style>
    ...
    .settings-collapse-chevron { color:var(--text-sub); font-size:1.02rem; line-height:1; flex:0 0 auto; width:1.1rem; text-align:center; }
...
(() => {
    ...
    function makeSettingsCardsCollapsible(){
        document.querySelectorAll('.view-settings .section-grid > .card').forEach((card,index)=>{
            ...
            card.dataset.collapsibleReady='1';const titleText=title.textContent.trim();const toggle=document.createElement('button');toggle.type='button';toggle.className='settings-collapse-toggle';toggle.innerHTML='<span class="settings-collapse-chevron" aria-hidden="true"></span><span class="settings-collapse-title"></span>';toggle.querySelector('.settings-collapse-title').textContent=titleText;
